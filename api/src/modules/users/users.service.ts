...
export class UsersService {
	...
	async findOne(id: number): Promise<User> {
		const user = await this.usersRepository.findOne({ where: { id }, relations: { userRole: true }, });
		if (!user) {
	...
	async updateResetToken(userId: number, resetPasswordToken: string, resetPasswordExpires: Date): Promise<void> {
	...
	async createAdmin(createAdminDto: CreateAdminDto, creatorId: number): Promise<User> {
	...
	async updateUserRole(userId: string, roleId: string, adminId: number): Promise<User> {
		const user = await this.usersRepository.findOne({ where: { userId } });
